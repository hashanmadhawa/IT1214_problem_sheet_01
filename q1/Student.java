...
class Classroom {
    private Student[] st_objects;
    private int studentCount;

    public Classroom() {
        st_objects = new Student[10];
        studentCount = 0;
    }

    public void addStudent(Student student) {
        if (studentCount < st_objects.length) {
            st_objects[studentCount++] = student;
        } else {
    ...
    public void updateAttendance(int studentId, int newDaysAttended) {
        for (int i = 0; i < studentCount; i++) {
            if (st_objects[i].getStudentId() == studentId) {
                st_objects[i].setDaysAttended(newDaysAttended);
                return;
            }
        }
    ...
    public void displayAllStudents() {
        for (int i = 0; i < studentCount; i++) {
            st_objects[i].displayStudentDetails();
        }
    }
}
